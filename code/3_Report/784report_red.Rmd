...
To examine gene expression across time before and after vaccination, mixed effect models are fitted on each gene where participants are treated as random effect and time is treated as fix effect. Further, time will be analyzed as a categorical variable. We will select participants who are sampled at all time points to have a balanced data, and use compound symmetry covariance structure for mixed effect model. Specifically, the mixed effect model is

$$ expr = \beta_0 + \beta_1I(time_{ij} = 1) + \beta_2I(time_{ij} = 3) + \beta_3I(time_{ij} = 14) + b_i + \varepsilon_{ij},$$
where $b_i \sim N(=,\sigma^_b)$ and $\varepsilon_{ij} \sim N(0, \sigma_e^2 I_4)$ and $b_i \perp \!\!\! \perp \varepsilon_{ij}$.
...
$$
Essentially, this test detects genes showing different expressions in at least one time point among day 0, 1, 3 and 14. P-values from Wald test are adjusted using Benjamini-Hotchberg (BH) method, and false discory rate is controlled at 1%. Then Gene Ontology (GO) enrichment analysis will be conducted on the DE genes. 

Next, we are interested in genes differentially expressed with respect to response to vaccination across time. 

## ADD HERE. Infer response part.

Again, we fit mixed effect model where time and the interaction between time and response. 
